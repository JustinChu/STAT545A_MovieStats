...
```
Noir and Animation seem to be the highest rated surprisingly. The only gender differnce I notice is maybe the fact that women seem to like musical more than men. The only reason fantasy and war seem like they make a difference is that the sample size for both is quite small.
...
* Horror movies do not get good ratings. It fits my expectations since horror movies tend to typically focused on cheap thrill but are not impactful.
...
I wanted to cluster these heatmaps also but I couldn't figure it out in ggplot2. Any tips on clustering with ggplot?
