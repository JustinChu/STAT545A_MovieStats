...
```
Most movies in the data tend to be from the 1990's. There is a pretty long tail meaning they have at least some moves from the past.
...
```
It is better to think of the number of movies as proportion rather than a total since some titles contribute multiple times to the count. It is worth noting that documentaries no longer seem to be a high count genre (as opposed to the previous plot). This is of course because movies that are documentaries typically do not have other genre associated with them.
...
```
Noir and Animation seem to be the highest rated surprisingly. Low sample sizes for some of these is a problem; the only reason fantasy and war seem like they make a difference in terms of gender is that the sample size for both is quite small. The only gender difference I notice is maybe the fact that women seem to like musicals more than men.
...
Below is a code to generate heatmap with genre and profession as the x and y axes. For simplicity I used the genre column I created in my previous script that merged multiple genres into the variable "multiple"). I removed elements that did not have very many observations (based on figure above).
...
### Testing out embedding images

Testing out embedding of image. It seems to work, but formatting is not exactly the same.
...
* grouplens.org/datasets/movielens/ - for posting there raw data publicly for people to analyze